 ...
 *
 * The citation encodes more than it looks. "18 Pa.C.S. § 2502" is title 18,
 * and the four-digit section number is really CHAPTER 25 plus SECTION 02 —
 * which the URL needs spelled out and zero-padded:
 *
 *   18 Pa.C.S. 2502  ->  /HTM/18/00.025.002.000..HTM
 *                                  ^^^ ch  ^^^ sec
 *
 * Nobody types that, which is the whole reason this is a resolver. Verified
 * across titles rather than assumed from one example: 18-2502 (murder),
 * 18-3502 (burglary) and 75-3802 (DUI) all resolve, and a bogus section
 * returns an honest 404.
 *
 * Note the host: the modern palegis.us front end renders statutes
 * client-side and returns a shell, while the older legis.state.pa.us path
 * still serves real HTML. The old path is the one that works.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Pennsylvania Consolidated Statutes');
}

const BASE = 'https://www.legis.state.pa.us/WU01/LI/LI/CT/HTM';
const UA = 'pipeworx-mcp-pennsylvania-code/1.0 (+https://pipeworx.io)';

const TITLES: Record<string, string> = {
  '18': 'Crimes and Offenses', '23': 'Domestic Relations', '42': 'Judiciary and Judicial Procedure',
  '75': 'Vehicles', '68': 'Real and Personal Property', '15': 'Corporations and Unincorporated Associations',
  '20': 'Decedents, Estates and Fiduciaries', '35': 'Health and Safety', '53': 'Municipalities Generally',
};

const tools: McpToolExport['tools'] = [{
  name: 'pa_consolidated_statute',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      title: { type: 'string', description: 'Pennsylvania title number, e.g. "18" (crimes), "75" (vehicles), "23" (domestic relations).' },
      section: { type: 'string', description: 'Section as cited, e.g. "2502" or "3802". The first two digits are the chapter.' },
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'pa_consolidated_statute') throw new Error(`Unknown tool: ${name}`);
  const title = String(args.title ?? '').trim().replace(/[^0-9]/g, '');
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '');
  const m = /^(\d{2})(\d{1,2})(?:\.(\d+))?$/.exec(raw);
  if (!title || !m) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.title} Pa.C.S. ${args.section}" is not a citation this can resolve.`,
      hint: 'Pennsylvania sections are four digits where the first two are the chapter, e.g. title "18" section "2502" (murder) or title "75" section "3802" (DUI).' };
  }
  const chapter = m[1].padStart(3, '0');
  const section = m[2].padStart(3, '0');
  const sub = (m[3] ?? '0').padStart(3, '0');
  const url = `${BASE}/${title}/00.${chapter}.${section}.${sub}..HTM`;

  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', title, section: raw,
      message: `No section ${raw} in title ${title} of the Pennsylvania Consolidated Statutes (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal. The first two digits must be a real chapter.' };
  }
  if (!res.ok) throw new Error(`Pennsylvania General Assembly returned HTTP ${res.status} for ${title} Pa.C.S. ${raw}`);

  const full = statuteText(await res.text());
  // The page leads with "Section 2502.0 - Title 18 - CRIMES AND OFFENSES
  // 18c2502s" — a breadcrumb and an internal anchor id — before the statute.
  // Anchor at the section symbol so the law leads.
  const at = full.indexOf(`§ ${raw}`);
  const text = at === -1 ? full : full.slice(at).trim();
  if (text.length < 40) {
    return { found: false, reason: 'empty_document', title, section: raw,
      message: `Title ${title} section ${raw} resolved but returned no usable text — it may be repealed or reserved.` };
  }
  const heading = (text.match(/^§\s*[\d.]+\.\s*([^\n.]{2,120})\./) ?? [])[1] ?? null;

  return {
    citation: `${title} Pa.C.S. § ${raw}`,
    title, title_name: TITLES[title] ?? null, section: raw, chapter: String(Number(m[1])),
    heading, text, text_chars: text.length, url,
    source: 'Pennsylvania General Assembly (legis.state.pa.us), keyless',
    attribution: 'Pennsylvania statutes are public record.',
  };
}
